...
def main():
    ...
    with open('ci/jobs/reboot-worker-management-dev.yml', 'w+') as pipeline:
        pipeline.write(template.render(config_data))
    with open('ci/jobs/mgmt-dev-user-creation.yml.j2') as in_template:
        template = jinja2.Template(in_template.read())
    with open('ci/jobs/mgmt-dev-user-creation.yml', 'w+') as pipeline:
        pipeline.write(template.render(config_data))
    with open('ci/jobs/mgmt-user-creation.yml.j2') as in_template:
        template = jinja2.Template(in_template.read())
    with open('ci/jobs/mgmt-user-creation.yml', 'w+') as pipeline:
        pipeline.write(template.render(config_data))
